...
async def scrape_with_firecrawl(url: str) -> str | None:
    ...
    try:
        ...
        # Check if result is valid and contains markdown
        if scrape_result and isinstance(scrape_result, dict):
            # Based on the example response, 'data' might contain an array of results
            if 'data' in scrape_result and isinstance(scrape_result['data'], list) and len(scrape_result['data']) > 0:
                first_result = scrape_result['data'][0]
                if 'markdown' in first_result:
                    markdown_content = first_result['markdown']
                else:
                    logger.warning(f"Unexpected data structure in Firecrawl response for {url}")
                    return None
            # Direct access if it's not in a data array
            elif 'markdown' in scrape_result:
                markdown_content = scrape_result['markdown']
            else:
                logger.warning(f"Unexpected response structure from Firecrawl for {url}")
                return None
            ...
            if markdown_content and isinstance(markdown_content, str):
                ...
                logger.warning(f"Firecrawl returned empty or invalid markdown for {url}")
                return None
        else:
            logger.warning(f"Firecrawl scrape did not return expected content for {url}. Result keys: {list(scrape_result.keys()) if isinstance(scrape_result, dict) else type(scrape_result)}")
            return None
        ...
        if any(x in str(e) for x in ["429", "Rate limit"]):
            ...
            try:
                ...
                # Check for markdown content in the response structure
                if scrape_result and isinstance(scrape_result, dict):
                    if 'data' in scrape_result and isinstance(scrape_result['data'], list) and len(scrape_result['data']) > 0:
                        first_result = scrape_result['data'][0]
                        if 'markdown' in first_result:
